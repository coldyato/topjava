...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "SET user_id=:userId, date_time=:dateTime, description=:description, " +
                "calories=:calories WHERE id=:id AND user_id=:userId", mealMap) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public Meal get(int id, int userId) {
        List<Meal> mealsFromDb = jdbcTemplate.query("SELECT * FROM meals WHERE id=? AND user_id=?", ROW_MAPPER, id, userId);
        return DataAccessUtils.singleResult(mealsFromDb);
    }
